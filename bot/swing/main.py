...
from config import symbol, timeframe, amount_usdt, leverage, per, trailingStopLoss
...
exchange = ccxt.binance({
    'apiKey': '1Cno4uAqF0XMZ5nNTCy2wSXL7wG4cXypnNVGnXdtELkadzhr0N2TJANVVqCRG7yV',
    'secret': 'Sc96uxWOVP2XboZZgsIOEASoaEnq1spMMTpUx1axvV2noZ3tAbPGkRBhpE4SAhVQ',
...
def long(symbol, amount_usdt):
    ...
    order = exchange.create_market_order(symbol, 'buy', amount_to_trade *leverage)
...
def short(symbol, amount_usdt):
    ...
    order = exchange.create_market_order(symbol, 'sell', amount_to_trade *leverage)
    ...
    while True:
        ...
        if trailing:
            ...
            if reject_change < -change*trailingStopLoss and last_signal == 'LONG':
                close_order =  asyncio.run(close_position(exchange, symbol, 'sell'))
                if close_order:
            ...
            if reject_change > change*trailingStopLoss and last_signal == 'SHORT':
                close_order =  asyncio.run(close_position(exchange, symbol,'buy'))
                if close_order:
                    log(f"LONG EXIT on trailing {reject_change} > {change*trailingStopLoss}")
                    log(str(close_order))
                    trailing = False
        ...
        # Trailing Enable                    
        if change > 10 and isOrderPlaced : trailing = True
