...
class Store:
    ...
    def setState(self, state, obj, tag=""):
...
def checkInSL(sls, symbol, _type, past = 20):
    inSL = False
    timePassed = False
    # Iterate through the list in 'SL'
    for item in sls:
        if symbol in item and item['type'] == _type:
            inSL= True
            # if not past: return inSL, True
            # Get the current time
            current_time = datetime.now()
            # past_time = current_time - timedelta(minutes=30)  # Convert to seconds
            past_time = datetime.fromtimestamp(item[symbol]/1000)
            # print(past_time)
...
# store.setState('scalp', {'SL':[{'ADA/USDT': 1729675760, 'type': 'SHORT'}]})
# key_exists = any('SOL/USDT' in item for item in state['SL'])


# sl = state['SL']
# sl.append({'TEST/USDT': 1729675760, 'type': 'SHORT'})
# store.setState('scalp', {'SL':sl})


# inSL, timePassed = checkInSL(state['SL'], 'ETH/USDT', 'LONG')
# print(inSL, timePassed)
...
# inSL_long, timePassed = checkInSL(state['SL'], activeSymbol, 'LONG')
# print(inSL_long)
